...
def admin_ID_check():
    while True:
        ...
        with open('admin_registration.txt','r') as user_name:
...
def deposit_money(user_ID):
    ...
    deposit_amount = float_input("Enter your deposit amount: ") # enter the deposit amount
    today_date = datetime.now().strftime("%Y-%m-%d")
...
def register_user():
    ...
    amount = float_input("Enter your starting amount: ")
    today_date = datetime.now().strftime("%Y-%m-%d")
...
def register_admin():
    ...
    with open('admin.txt', 'a') as user_login:
        user_login.write(f"{admin_ID}, admin name: {admin_name}, password: {password}\n")
...
def transaction_history(user_ID):
    ...
    while True:
        ...
        if choice == 1:
            print("-"*100)
            print(f"{'user_ID'.ljust(10)} {'Date'.ljust(21)} {'type'.ljust(25)} {'reason'.ljust(25)} {'balance'.rjust(10)}")
            print("-"*100)
            for j in transaction:
                # print(f"{j[0]:<10} {j[1]:<20} {j[2]:<15} {j[3]:<25} {j[4]:>10}")
                print(f"{j[0].ljust(10)} {j[1].ljust(20)} {j[2].ljust(25)} {j[3].ljust(25)} {j[4].rjust(10)}")
            ...
            for k in transaction:
                if date_input == k[1]:
                    check = True
                    print(f"{k[0].ljust(10)} {k[1].ljust(20)} {k[2].ljust(25)} {k[3].ljust(25)} {k[4].rjust(10)}")
            if check == False:
...
def change_admin_name_password():
    admin_info = admin_ID_check() # admin ID, original name
    ...
    for i in x:
        z = i.strip().split(",")[0].strip()
        a[z] =  [i.strip().split(",")[1].split(':')[1].strip(),  i.strip().split(",")[2].split(':')[1].strip()] # store data to the dictionary
        if admin_ID == z:
    ...
    for key,values in a.items():
        if b == 0:
            with open('admin.txt', 'w') as update:
                update.write(f"{key}, admin name: {values[0]}, password: {values[1]}\n") # over write all data
            b = 1
        else:
...
def super_admin_login():
    ...
    Password = "S1001"
    super_admin_name = input("Enter the admin name(S1001) :- ")
    password = input("Enter the admin password(S1001) :- ")
